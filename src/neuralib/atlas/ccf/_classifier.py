...
class RoiClassifierDataFrame(DataFrameWrapper):
    ...
    def with_parsed(self, *,
        ...
        if self._cached_dir is not None:
            ...
            if file.exists() and not force:
                ...
                ret = self._parse(filter_capital, tree, family, hemisphere, copy_overlap, filter_injection)
    ...
    def _parse(self, filter_capital, tree, family, hemisphere, copy_overlap, filter_injection) -> Self:
        ret = self
        if filter_capital:
...
class RoiNormalizedDataFrame(DataFrameWrapper):
    ...
    def to_winner(self, sources: Sequence[str]) -> pl.DataFrame:
        ...
        df = (
            self.dataframe()
            .pivot(values='counts', on='source', index=self.classified_column, aggregate_function='first')
            .fill_nan(0)
            .fill_null(0)
